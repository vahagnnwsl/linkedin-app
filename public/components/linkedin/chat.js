Vue.component('linkedin-chat', {
            ...
            <div class="chat-container">
                <div class="card direct-chat direct-chat-primary p-1" style="background-color: #f7f7f7">
                    <div class="card-header" v-if="selectedConversation.connection">
                        ...
                        <span class="badge badge-secondary mr-2" @click="selectRelativeConversation(rel)" style="cursor: pointer" v-for="rel in relativeConversations">{{rel.account.full_name}}</span>
                    ...
                    <div class="modal" id="relativeConversationModal">
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <div class="direct-chat-messages" id="messages" style="min-height: 690px;overflow-y: scroll">
                                        ...
                                        <div class="direct-chat-msg" v-for="(message,key) in selectedRelativeConversationMessages" :key="key"
                                    ...
                                    <button v-if="selectedRelativeConversationMessages.length" @click="relativeConversationMessages(selectedRelativeConversation)"
                                            class="btn btn-default"
                                            style="float: right" title="load more messages"><i class="fa fa-arrow-circle-down"></i>
                                    </button>
                                </div>
    ...
    data: function () {
        return {
            ...
            selectedRelativeConversation: [],
            selectedRelativeConversationMessages: [],
            relativeConversationMessagesStart: 0,
            start: 0,
            messageStart: 0,
            searchKey: '',
            form: {
    ...
    methods: {
        ...
        selectRelativeConversation:function (conversation){
            this.relativeConversationMessages(conversation);
            $('#relativeConversationModal').modal('show');
            this.selectedRelativeConversation = conversation;
        },
        relativeConversationMessages:function (conversation){

            this.$http.get(`/dashboard/conversations/${conversation.id}/messages?start=${this.relativeConversationMessagesStart}`)
                .then((response) => {
                    this.selectedRelativeConversationMessages.unshift(...response.data.messages)

                    this.relativeConversationMessagesStart += 10;
                })
        },
        ...
        selectConversation: function (conversation) {
            this.relativeConversations = [];
            this.selectedConversation = [];
            this.relativeConversationMessagesStart = 0;
            this.messageStart = 0;
            this.messages = [];
        ...
        getConversationMessages: function (id) {
            ...
            this.$http.get(`/dashboard/conversations/${id}/messages?start=${this.messageStart}&relative=true`)
                .then((response) => {

                    this.messages.unshift(...response.data.messages)
                    this.messageStart += 10;

                    this.relativeConversations=response.data.relatedConversations;
                })
        },
        scroll: function () {
            setTimeout(function () {
