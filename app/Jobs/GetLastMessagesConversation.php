...
class GetLastMessagesConversation implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($response['success']) {
            $this->messageRepository->updateOrCreateCollection($response['data'], $this->conversation->id, $this->user->id, $this->account->id, $this->account->entityUrn, $this->messageRepository::SENDED_STATUS, $this->messageRepository::RECEIVE_EVENT,true);
        }
    }
}
